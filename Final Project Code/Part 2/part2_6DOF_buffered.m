function U = ROB599_ControlsProject2_part2_Team9(TestTrack, Xobs)
TIMER = tic;
MAX_TIME_SEC = 870; % 14.5 minutes * 60 seconds = 870
%close all
%load TestTrack.mat
...
TestTrack2.leftlane = TestTrack2.cline - (TestTrack2.cline - TestTrack2.bl)./3;
TestTrack2.rightlane = TestTrack2.cline + (TestTrack2.br - TestTrack2.cline)./3;
...
Xobs_lane = zeros(length(Xobs),1);
Xobs_index = zeros(length(Xobs),1);
closestCenterPoint = zeros(length(Xobs),1);
for k = 1:length(Xobs)
    %ob = Xobs{k};
    ob_c = Xobs_c(k,:);
    %ob_r = Xobs_r(k);
    ...
    [cdist, cindex] = closestPoint(ob_c', TestTrack.cline);
...
%     if x(i,1) > 1225
%         TestTrack2.leftlane = TestTrack2.cline - (TestTrack2.cline - TestTrack2.bl)./2;
%     end
%     if x(i,1) > 950
%         TestTrack2.rightlane = TestTrack2.cline + (TestTrack2.br - TestTrack2.cline)./2;
%     end
    ...
    if x(i,3) < 0
        if x(i,3) > TestTrack2.cline(2,(closestCenterPoint(next_obs,1)))
            next_obs = next_obs + 1;
        end
...
%         end
    elseif x(i,3) > 0
        if x(i,1) > TestTrack2.cline(1,(closestCenterPoint(next_obs,1)))+0.25 && next_obs < length(Xobs)
        %if x(i,1) > Xobs_c(next_obs,1)+0.25 && next_obs < length(Xobs)
           %if x(i,5) > 0    % If psi > 0
               %if x(i,3) > Xobs_c(next_obs,2)   % If y greater than obs_center
                next_obs = next_obs + 1;
               %end
%            elseif x(i,5) < 0    % If psi < 0
%                if x(i,3) < Xobs_c(next_obs,2)   % If y less than obs_center
%                 next_obs = next_obs + 1;
%                end
%            end
        end
